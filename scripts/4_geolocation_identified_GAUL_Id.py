...
geonames_locations = pd.read_csv(get_path("geonames_locations_clean_path")).drop(columns={"Unnamed: 0"})
...
events_adm1 = events_adm1[["ADM1_NAME", "ADM1_CODE", "ADM2_NAME", "ADM2_CODE","DisNo.","Location","geoNames","Province","ISO","admin_level","geocoding_q"]]
...
events_adm2 = events_adm2[["ADM1_NAME", "ADM1_CODE", "ADM2_NAME", "ADM2_CODE","DisNo.","Location","geoNames","Province","ISO","admin_level","geocoding_q"]]
...
selected_disasters = disaster_types[(disaster_types["Disaster Type"] == 'Drought') | (disaster_types["Disaster Type"] == 'Extreme temperature') | (disaster_types["Disaster Type"] == 'Flood') | (disaster_types["Disaster Type"] == 'Storm')  | (disaster_types["Disaster Type"] == 'Wildfire') | (disaster_types["Disaster Type"] == 'Mass movement (wet)') | (disaster_types["Disaster Type"] == 'Mass movement (dry)')]

selected_disasters = selected_disasters.rename(columns={"Disaster Type" : "disaster_type"}).drop(columns={"Disaster Subtype"})
...
climate_event_locations_90_23 = pd.concat([admin1_locations, admin2_locations]).reset_index().drop(columns={"disaster_type","index"})
...
climate_event_locations_90_23 = gpd.GeoDataFrame(climate_event_locations_90_23, crs="EPSG:4326")
...
#climate_event_locations_90_23.geometry = climate_event_locations_90_23.geometry.apply(lambda geometry: functions.fix_invalid_geometry(geometry))
...
for even in np.unique(climate_event_locations_90_23["DisNo."]):
    location_gdf = climate_event_locations_90_23[climate_event_locations_90_23["DisNo."] == even]
...
for i in np.arange(len(cleaned_results)):
    df = climate_event_locations_90_23[climate_event_locations_90_23["DisNo."] == cleaned_results[i][0]].drop(columns="geometry")
    duplicated_df = df[((df['ADM1_CODE'].duplicated(keep=False)) & (df['admin_level'] == 1))]
...
climate_event_locations_90_23 = climate_event_locations_90_23.reset_index().drop(columns={"index"})

#write identified locations
climate_event_locations_90_23.to_file(get_path("intermediate_data_path")+"geolocated_climate_events_1990-2023_simplified_clean.gpkg", driver="GPKG")
