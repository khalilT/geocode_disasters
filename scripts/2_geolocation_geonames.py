...
existing_files = [f for f in os.listdir(output_dir) if f.startswith("EMDAT_golocations_coordinates_q")]
...
for j in range(start_index, len(df_locations)):
    ...
    result.update({event[1]: functions.get_place_coordinates_geonames(event[1].strip(), event[0], event[2], username)})
    event_df = pd.DataFrame.from_dict(result, orient='index')
    event_df.columns = ['DisNo.', 'EM-DAT-Location', 'Longitude', 'Latitude', 'geoNames', 'Province']
    ...
    if (j + 1) % chunk_size == 0 or (j + 1) == len(df_locations):
        ...
        results_locations.to_csv(f"{output_dir}/EMDAT_golocations_coordinates_q{file_number}.csv", index=False)
